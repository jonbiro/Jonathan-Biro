...
const Work = () => (
    <section id="work" tabIndex={-1} className="mx-auto max-w-7xl px-4 py-10 md:py-14">
        <div className="flex flex-wrap items-end justify-between gap-4 border-t border-slate-200 pt-8">
            <div><p className="section-eyebrow">Portfolio</p><h2 className="section-title mt-3">Selected projects</h2></div>
            <p className="max-w-sm text-sm leading-relaxed text-slate-600">Independent projects, with the decisions and source evidence behind them.</p>
        </div>

        <article className="mt-8 overflow-hidden rounded-lg border border-slate-200 bg-white">
            <div className="grid lg:grid-cols-[1.1fr_1fr]">
                <figure className="border-b border-slate-200 bg-[#e8edf0] lg:border-b-0 lg:border-r">
                    ...
                    <figcaption className="flex justify-between gap-3 bg-slate-100 px-5 py-3 text-xs text-slate-600"><span>BiroMD · Live homepage</span><span>Captured September 2026</span></figcaption>
                </figure>
                <div className="p-6 sm:p-8">
                    <p className="section-eyebrow">Featured case study / Healthcare</p>
                    <h3 className="mt-3 text-3xl font-bold">BiroMD</h3>
                    <p className="mt-4 text-base leading-relaxed text-slate-700">A medical-practice website with automated validation of patient navigation, consultation links, and accessible interface states.</p>
                    <dl className="mt-6 grid grid-cols-2 gap-4 text-sm">
                        <div><dt className="text-slate-500">Contribution</dt><dd className="mt-1 text-slate-800">Frontend delivery and quality engineering</dd></div>
                        <div><dt className="text-slate-500">Project type</dt><dd className="mt-1 text-slate-800">Independent medical-practice website</dd></div>
                    </dl>
            ...
            <div className="grid gap-6 border-t border-slate-200 p-6 sm:p-8 md:grid-cols-3">
...
expect(await textContrast(cta))
  .toBeGreaterThanOrEqual(4.5);`}</code></pre>
                        <p className="mt-3 text-sm leading-relaxed text-slate-600">Repository test excerpt. Live deployment results may vary by revision.</p>
                    </div>
                </div>
            </details>
            <details className="case-details">
                ...
                <ol className="grid gap-4 pb-7 pt-3 sm:grid-cols-3">
                    {[["Build", "Generate the static site and authorized public image derivatives."], ["Verify", "Check required routes, sitemap entries, internal links, and image budgets."], ["Exercise", "Run browser scenarios for consultation, navigation, themes, and gallery behavior."]].map(([title,body],i)=><li key={title} className="rounded-xl bg-sky-50 p-4"><span className="text-sm text-sky-700">0{i+1}</span><h4 className="mt-2 font-semibold">{title}</h4><p className="case-copy">{body}</p></li>)}
                </ol>
